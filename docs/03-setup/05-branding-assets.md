...
| `assets/feature-graphic.svg` | 1024×500 | Play Store feature graphic banner (moon + title) |

**Play Store assets** (generated, do not hand-edit):

| File | Size | Play Console field |
|------|------|-------------------|
| `play-store/icon-512.png` | 512×512 | High-res icon |
| `play-store/feature-graphic.png` | 1024×500 | Feature graphic |

Regenerated by `npm run assets:generate` from the SVG sources above.
...
This runs `@capacitor/assets` and post-steps:

- `scripts/patch-android-adaptive-icon.cjs` — writes `drawable/ic_launcher_moon.xml` (vector) and wires API 26+ adaptive icons to it — avoiding blurry scaled PNG mipmaps on home screens.
- `scripts/generate-play-store-assets.cjs` — writes `play-store/icon-512.png` and `play-store/feature-graphic.png` for Play Console upload.

- `android/app/src/main/res/mipmap-*` — launcher icons
- `android/app/src/main/res/drawable-*` — splash drawables
- `play-store/icon-512.png` — Play Store high-res icon
...
4. **Play Store icon** — `play-store/icon-512.png` is 512×512 PNG, under 1 MB
5. **Play Store feature graphic** — `play-store/feature-graphic.png` is 1024×500 PNG, under 15 MB
